...
enum
{
    lay_obs=-1000,                                                                                  // this is arbitrary, but shouldn't be positive, since there can conceivably be any positive number of layers
    lay_zone,
    lay_ent
};

struct SAutoTileInfo
{
    int nTile;                                                                                      // the tile being set
    int nTop,nBottom,nLeft,nRight;
    int nTopleft,nTopright;
    int nBottomleft,nBottomright;
};
...
class CMapView
{
    ...
    {
        mode_normal,
        mode_autoset,                                                                               // setting tiles, with experimental automatic border tiling stuff
...
public:
    ...
    bool    bMouseleft,bMouseright,bMousemid;                                                       // mouse button flags
    
    SAutoTileInfo autotileinfo;

    std::vector<bool> bLayertoggle;                                                                 // each element is true if the specified layer is to be rendered.  False otherwise
    bool bZonetoggle,bEnttoggle,bObstoggle;                                                         // true if the zone, entities, and obstructions are to be rendered, respectively
    
    RECT    clientrect;
    Rect    curselection;                                                                           // the current active selection rectangle.

// ----------------- PNG ---------------------
// This was originally in the Sphere source, so, give aegis credit for this.  I just hacked it up a bit. --tSB
// blegh, so ugly
...
namespace
{
    ...
    // libpng file i/o callback functions

    void __cdecl PNG_read_function(png_structp png_ptr, png_bytep data, png_size_t length)
    {
        File* file = (File*)png_get_io_ptr(png_ptr);
        file->Read(data,length);
    }
    
    void __cdecl PNG_write_function(png_structp png_ptr, png_bytep data, png_size_t length)
    {
        File* file = (File*)png_get_io_ptr(png_ptr);
        file->Write(data, length);
    }
    
    void __cdecl PNG_flush_function(png_structp png_ptr)
    {
        // files automatically flush
    }
};

bool PNG::Load(CPixelMatrix& dest,const char* filename)
{
    png_image* pis=0;
    ...
    dest.CopyPixelData(pis->pixels,pis->width,pis->height);
    delete pis;
    return true;
    //return pis;
}

bool PNG::Save(CPixelMatrix& src,const char* filename)
{
    File file;
    ...
    for (int i = 0; i < src.Height(); i++)
    {
        ...
        memcpy(rows[i], src.GetPixelData() + i * src.Width(), src.Width() * sizeof(RGBA));
    }

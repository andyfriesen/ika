...
PyObject* CScriptEngine::Entity_GetAttribute(PyObject* self,char* name)
{
	...
	if (!strcmp(name,"sprite"))
		return PyString_FromString(pEngine->characters[pEnt->hSprite].sFilename.c_str());
	if (!strcmp(name,"hotx"))
		return PyInt_FromLong(pEngine->characters[pEnt->hSprite].hotx);
	if (!strcmp(name,"hoty"))
		return PyInt_FromLong(pEngine->characters[pEnt->hSprite].hoty);
	if (!strcmp(name,"hotwidth"))
		return PyInt_FromLong(pEngine->characters[pEnt->hSprite].hotw);
	if (!strcmp(name,"hotheight"))
		return PyInt_FromLong(pEngine->characters[pEnt->hSprite].hoth);
...
int CScriptEngine::Entity_SetAttribute(PyObject* self,char* name,PyObject* value)
{
	...
	if (!strcmp(name,"sprite"))
	{
		pEnt->hSprite=pEngine->characters.Load(PyString_AsString(value));
...
METHOD(entity_detectcollision)
{
	...
	int x1=e1->x;
	int y1=e1->y;
	int x2=x1+pEngine->characters[e1->hSprite].hotw;
	int y2=y1+pEngine->characters[e1->hSprite].hoth;
	...
	while (PyDict_Next(pEntitydict,&nCount,&pKey,&pValue))
	{
		...
		if (x1>e2->x+pEngine->characters[e2->hSprite].hotw)	continue;
		if (y1>e2->y+pEngine->characters[e2->hSprite].hoth)	continue;
		if (x2<e2->x)	continue;
		if (y2<e2->y)	continue;
...
METHOD(std_spawnentity)
{
	...
	int hSprite=pEngine->characters.Load(chrname);

	pEngine->entities[nEnt].sName="Spawned entity";
	pEngine->entities[nEnt].hSprite=hSprite;

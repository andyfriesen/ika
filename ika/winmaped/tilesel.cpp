...
void CTileSel::UpdateVSP(int tileidx)
{
	if (tileidx<0 || tileidx>=pVsp->NumTiles())				// -1 is below 0, so the default argument's value is already handled here as it is.
	{
		...
		{
			tileimages[i].CopyPixelData(pVsp->GetPixelData(i),pVsp->TileX(),pVsp->TileY(),pVsp->ColourDepth(),pVsp->Pal());
			tileimages[i].SetBitDepth(4);
		}
	}
	else
	{
		tileimages[tileidx].CopyPixelData(pVsp->GetPixelData(tileidx),pVsp->TileX(),pVsp->TileY(),pVsp->ColourDepth(),pVsp->Pal());
		tileimages[tileidx].SetBitDepth(4);
	}
...
bool CTileSel::ExportPNG(const char* fname)
{
	...
	bigImage.SetBitDepth(pVsp->ColourDepth());
	bigImage.Resize(nPixwidth,nPixheight);
	bigImage.Clear((255<<24)|(255<<8));
	...
	for (int i=0; i<pVsp->NumTiles(); i++)
	{
		curtile.CopyPixelData(pVsp->GetPixelData(i),pVsp->TileX(),pVsp->TileY(),pVsp->ColourDepth(),pVsp->Pal());

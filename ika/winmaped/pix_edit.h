...
protected:
	...
	// TODO: replace these with linked lists
	...
	void DoLeftDownOnBigTile(int x,int y,int b,int& nCurcolour);
	void DoLeftUpOnBigTile(int x,int y,int b);
	
	void DoLeftDownOnSwatch(int x,int y,int b,int& nCurcolour);
	...
	int  CalcColour(int nLumina,int nChroma);
	int  SetAlpha(int nAlpha, int nColor);
	int  MergeColors(int nColor1, int nColor2);
	void UpdateLumina(HWND hBar);
	void UpdateAlpha(HWND hBar);
	void MakeSwatchImage();
	
	int		nChroma;			// hue of the current color
	int		nLumina;			// light
	int		nAlpha;				// translucency
	int		nCurcolour[2];		// Current colour index

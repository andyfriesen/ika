...
Direction Entity::MoveDiagonally(Direction d) {
    ...
    if (obstructedByMap && engine.DetectMapCollision(x, newy, sprite->nHotw, sprite->nHoth, layerIndex)) {
        ...
        obstructedByEntities && 
        engine.DetectEntityCollision(this, 
            x, newy, sprite->nHotw, sprite->nHoth, 
            layerIndex, true)
    ) {
        d1 = face_nothing;
    }
...
void Entity::Move(Direction d) {
    ...
    if (obstructedByEntities) {
        Entity* ent = engine.DetectEntityCollision(this, newx, newy, sprite->nHotw, sprite->nHoth, layerIndex, true);
...
void Entity::GetMoveScriptCommand() {
    if (moveScript) {
        ...
        delayCount = 1000000;
        // whatever.  The idea here is that the entity does nothing, 
        // so we delay for, say, about 3 hours, then check again.
    }
}
